...
const char* FIRMWARE_VER = "v2.0-dev";
...
class LGFX_Waveshare35 : public lgfx::LGFX_Device {
    ...
    lgfx::Touch_FT5x06 _touch_instance;   // v2: FT6336 uses FT5x06-compatible protocol
...
public:
    LGFX_Waveshare35(void) {
        ...
        {
            // v2: FT6336 capacitive touch panel on the shared I2C bus.
            // Reset and INT are routed through TCA9554 (EXIO1 = LCD_RST,
            // shared with the panel reset pulse; EXIO2 = TP_INT, polled
            // instead of interrupt-driven). bus_shared=true so LovyanGFX
            // doesn't re-init the I2C bus already brought up in setup()
            // via Wire.begin(8, 7).
            auto cfg = _touch_instance.config();
            cfg.x_min          = 0;
            cfg.x_max          = LCD_HOR_RES - 1;
            cfg.y_min          = 0;
            cfg.y_max          = LCD_VER_RES - 1;
            cfg.pin_int        = -1;
            cfg.pin_rst        = -1;
            cfg.bus_shared     = true;
            cfg.offset_rotation = 0;
            cfg.i2c_port       = 0;
            cfg.i2c_addr       = 0x38;      // FT6336 default
...
void loop() {
    ...
    // ---- v2.0 Phase A: Touch diagnostic ----
    // While on the idle screen, poll the FT6336 once per loop and log any
    // press to Serial. Rate-limited to one log line per 250 ms so a
    // sustained touch doesn't flood the monitor. Will be replaced by the
    // gesture detector in Phase B.
    {
        static unsigned long _lastTouchLogMs = 0;
        uint16_t tx = 0, ty = 0;
        if (gfx.getTouch(&tx, &ty)) {
            unsigned long now2 = millis();
            if (now2 - _lastTouchLogMs >= 250) {
                _lastTouchLogMs = now2;
